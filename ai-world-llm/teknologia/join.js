...
async function mergeFiles(selectedFiles, currentDir, outputFilePath) {
  let mergedContent = '';

  for (const filePath of selectedFiles) {
    const fileContent = await fs.promises.readFile(filePath, 'utf-8');
    const sectionHeader = `${filePath.split(currentDir)[1]} CODE IS BELOW\n`;
...
function getTimestampedFileName() {
  ...
  return `ollama-code-analyzer-join.txt`;
}
...
async function main() {
  ...
  await mergeFiles(selectedFiles, currentDir, outputFilePath);
